...
final class MenuBarItemTagNamespaceTests: XCTestCase {
    ...
    func testContinuumNamespace() {
        let continuum = MenuBarItemTag.Namespace.continuum
        XCTAssertTrue(continuum.isString)
        XCTAssertEqual(continuum.description, Constants.bundleIdentifier)
    }
...
final class MenuBarItemTagTests: XCTestCase {
    ...
    func testIsSystemItemForContinuum() {
        let tag = MenuBarItemTag(
            namespace: .continuum,
            title: "SomeItem"
        )
