...
/// (1) Continuum icon, (2) non-hideable system item, (3) new hideable item,
...
final class PlanLeftmostMoveTests: XCTestCase {
    ...
    /// The Continuum visible-control icon left of the divider triggers the
    /// Continuum-icon recovery branch.
    func testContinuumIconLeftOfDividerTriggersContinuumIconBranch() {
        let continuum = leftmostItem(
        ...
        let decision = LayoutSolver.planLeftmostMove(
            ...
            observation: LayoutSolver.LeftmostObservation(
                ...
                sectionByWindowID: [continuum.windowID: .hidden],
                previousWindowIDs: []
            ),
            savedSectionOrder: [:],
        ...
        if case let .controlIcon(item) = decision {
            XCTAssertEqual(item.windowID, 700)
        } else {
            XCTFail("expected .controlIcon, got \(decision)")
        }
    }

    /// The real menu bar can report the Continuum icon overlapping the
    /// expanded hidden divider by a point. Section classification still
    /// knows it is hidden; the recovery branch should not require a strict
    /// maxX <= hidden.minX geometry check in that case.
    func testContinuumIconStraddlingExpandedDividerStillTriggersRecovery() {
        let hiddenBounds = CGRect(x: -4_030, y: 0, width: 5_006, height: 33)
        let continuum = MenuBarItem.fixture(
            tag: .visibleControlItem,
            windowID: 710,
    ...
    /// A non-hideable system indicator (camera / mic / capture)
    ...
    func testNewWindowIDWithUnresolvedSourcePIDStillShortCircuits() {
        let newApp = leftmostItem(
            tag: appTag("com.example.unresolved", "Status"),
            x: 100,
    ...
    func testUnresolvedControlCenterPlaceholderIsIgnoredAsNewItemCandidate() {
        let placeholder = leftmostItem(
        ...
        let decision = LayoutSolver.planLeftmostMove(
            ...
            observation: LayoutSolver.LeftmostObservation(
                hiddenBounds: hiddenBounds,
                sectionByWindowID: [placeholder.windowID: .visible],
                previousWindowIDs: [101, 102, 103]
            ),
            savedSectionOrder: [
                "hidden": ["com.wireguard.macos:Item-0"],
            ],
