...
final class MarkerPairResolverTests: XCTestCase {
    ...
    private let continuumBundleID = "com.thepatientzero.Continuum"
    private let ccBundleID = "com.apple.controlcenter"
    private let continuumControlItemPrefix = "Continuum.ControlItem."
    ...
    /// Marker's CG owner is the widget's real app (not CC, not Continuum):
    ...
    /// Marker's CG owner resolves to Continuum itself: rejected, falls
    /// through to title lookup. The title-lookup result must also
    /// be checked for Continuum self-attribution (see the next test).
    func testContinuumOwnerFallsThroughToTitleLookup() {
        let icons = [icon(windowID: 1, title: "Item-0")]
        ...
        let result = MarkerPairResolver.resolve(
            ...
            ccBundleID: ccBundleID,
            pidToBundleID: { pid in
                if pid == 100 { return self.continuumBundleID }
                if pid == 777 { return "com.example.widget" }
                return nil
            },
            bundleIDToPID: { bundleID in
    ...
    /// Both paths resolve to Continuum: no resolution emitted. Defensive
    /// guarantee that Continuum's own PID is never attributed to a
    /// third-party widget regardless of where the lookup happens to
    /// land.
    func testBothPathsResolveToContinuumProducesNoResult() {
        let icons = [icon(windowID: 1, title: "Item-0")]
        let markers = [marker(windowID: 2, title: "com.example.widget", owningPID: 100)]
        let result = MarkerPairResolver.resolve(
            unresolvedIcons: icons,
            ...
            continuumBundleID: continuumBundleID,
            ccBundleID: ccBundleID,
            pidToBundleID: { _ in self.continuumBundleID },
    ...
    func testExtractMarkersExcludesGenericTitles() {
        ...
        let markers = MarkerPairResolver.extractMarkers(
            from: windows,
            continuumControlItemPrefix: continuumControlItemPrefix,
            continuumBundleID: continuumBundleID
        )
        XCTAssertEqual(markers.map(\.windowID), [4])
    }

    /// Continuum control items are excluded by the Continuum.ControlItem.
    /// prefix even though their titles contain dots.
    func testExtractMarkersExcludesContinuumControlItems() {
        let windows: [(windowID: CGWindowID, title: String?, size: CGSize, owningPID: pid_t?)] = [
            (1, "Continuum.ControlItem.Hidden", CGSize(width: 5016, height: 33), nil),
            (2, "Continuum.ControlItem.AlwaysHidden", CGSize(width: 5016, height: 33), nil),
    ...
    /// The Continuum self-registration window (title equals the Continuum bundle
    /// identifier) is excluded so Continuum's own PID can never be
    /// attributed to a third-party widget via the title-lookup path.
    func testExtractMarkersExcludesContinuumSelfRegistration() {
        let windows: [(windowID: CGWindowID, title: String?, size: CGSize, owningPID: pid_t?)] = [
            (1, "com.thepatientzero.Continuum", CGSize(width: 33, height: 33), nil), // Continuum self
