...
final class MenuBarSectionNameTests: XCTestCase {
    ...
    func testPresentationModeFallsBackToOverlayTrayWhenItemsDoNotFitAndHidingMenusIsDisabled() {
        let mode = MenuBarSection.presentationMode(
        ...
        XCTAssertEqual(mode, .overlayTray)
    }

    func testPresentationModeHidesApplicationMenusBeforeUsingOverlayTray() {
        let mode = MenuBarSection.presentationMode(
            totalItemsWidth: 1000,
            appMenuRightEdge: 350,
            screenFrameMinX: 0,
    ...
    func testPresentationModeStillUsesOverlayTrayWhenItemsCannotFitEvenAfterHidingMenus() {
        let mode = MenuBarSection.presentationMode(
