...
/// pure filter Phase 3 of applySavedSectionLayout uses to decide which UIDs
/// flow into planUnmanagedPlacement.
///
/// Pins down two invariants the field-reported "Continuum icon keeps moving"
/// regression turned out to depend on:
///
/// 1. All three Continuum control items (hidden, alwaysHidden, visible) are
...
final class PartitionUnmanagedUIDsTests: XCTestCase {
    ...
    func testAllThreeControlItemsExcluded() {
        let hidden = "com.thepatientzero.Continuum:Continuum.ControlItem.Hidden"
        let ah = "com.thepatientzero.Continuum:Continuum.ControlItem.AlwaysHidden"
        let visible = "com.thepatientzero.Continuum:Continuum.ControlItem.Visible"
    ...
    /// savedSectionOrder or the saved layout) are excluded. Only items
