...
/// applySavedSectionLayout: unmanaged items overflow before layout items,
...
final class PlanNotchOverflowTests: XCTestCase {
    ...
    /// Build a desiredFiltered sequence for: chevron + visible saved-layout
    ...
    private let chevron = "continuum:VisibleControlItem"
    private let hiddenCtrl = "continuum:HiddenControlItem"
    private let ahCtrl = "continuum:AlwaysHiddenControlItem"

    // MARK: - Scenarios

    /// When the saved layout fits and there are no unmanaged items, overflow
    /// is empty and inputs pass through unchanged.
    func testSavedLayoutFitsNoUnmanagedNoOverflow() {
    ...
    /// Saved layout fits, unmanaged also fits — no overflow.
    func testSavedLayoutFitsUnmanagedFitsNoOverflow() {
    ...
    /// Saved layout fits, but unmanaged doesn't — overflow only unmanaged,
    /// leftmost-first (chevron-side first).
    func testSavedLayoutFitsUnmanagedOverflowsLeftmostFirst() {
    ...
    /// Saved-layout baseline exceeds the budget: all unmanaged overflow,
    /// then saved-layout leftmost overflows until the remainder fits.
    func testSavedLayoutBaselineExceedsBudgetOverflowsAllUnmanagedThenLeftmostSaved() {
        // chevron(24) + p1(24) + p2(24) + p3(24) + u1(24) = 120
        // Available 70: savedBaseline = 24 + 24 + 24 + 24 = 96 > 70.
        // All unmanaged overflow (u1).
        // From CC end, fit layout items: chevron(24) + p3(24) = 48 <= 70 ✓
        //                                  + p2(24) = 72 > 70 ✗
    ...
    /// Tiered priority: an unmanaged item to the RIGHT of layout items
    /// still overflows before any layout item, because the tier check
    /// runs before the leftmost-first ordering.
    func testTieredPriorityUnmanagedOverflowsBeforeSavedLayout() {
        // chevron(24) + p1(24) + p2(24) + u1(24) = 96
        // Available 80: savedBaseline = 24 + 24 + 24 = 72 <= 80. Saved layout fits.
        // Try fitting unmanaged: usedWidth=72 + u1=24 = 96 > 80 — u1 doesn't fit.
        // So u1 overflows, saved layout stays.
        let desired = makeSequence(
            chevron: chevron,
            visible: ["p1", "p2", "u1"],
            hiddenCtrl: hiddenCtrl,
    ...
    /// "saved layout alone exceeds budget" branch ejects every visible item, which
