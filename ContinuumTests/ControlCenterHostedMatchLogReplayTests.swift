...
final class ControlCenterHostedMatchLogReplayTests: XCTestCase {
    ...
    /// Control Center; it has to stay unresolved so marker-pair resolution can
    /// still find the owning app.
    ...
    /// unresolved so marker-pair resolution can still find the owning app.
    ...
    /// Clock, or Continuum's own items) — or to no known app at all — is never a bare
    /// CC slot.
    func testNonControlCenterMatcherIsNeverASlot() {
        for matcher in ["com.fabriceleyne.theclock", "com.thepatientzero.Continuum"] {
            XCTAssertFalse(
