...
/// Accessibility are thread blocking, callers should perform resolution
/// off the main actor.
final class SourcePIDCache: @unchecked Sendable {
    ...
    private struct State {
        ...
        // Accessibility identifier of the matched extras-menu-bar child
        // (e.g. "com.apple.menuextra.wifi"). Control Center modules carry no
        // CG window title without Screen Recording, but expose a stable
        // AXIdentifier here, which lets us name them (Wi-Fi, Bluetooth, …).
        var axIdentifiers = [CGWindowID: String]()
    ...
    enum SpatialMatchPolicy {
        static let strictRadius: CGFloat = 1
        static let clearNearestRadius: CGFloat = 4
        static let clearNearestMinimumSeparation: CGFloat = 12
        static let clearNearestCandidateRadius = clearNearestRadius + clearNearestMinimumSeparation

        static func acceptsClearNearest(
            bestDistance: CGFloat,
            secondDistance: CGFloat?,
            bestBundleID: String?,
            windowTitle: String?,
            controlCenterBundleID: String
        ) -> Bool {
            guard bestDistance <= clearNearestRadius else {
                return false
            }

            let nearestCompetitor = secondDistance ?? .greatestFiniteMagnitude
            guard nearestCompetitor - bestDistance >= clearNearestMinimumSeparation else {
                return false
            }

            guard bestBundleID == controlCenterBundleID else {
                return true
            }
            ...
            return !MarkerPairResolver.isGenericControlCenterTitle(windowTitle)
        }
    }
    ...
    func pid(for window: WindowInfo) -> pid_t? {
        // Wrap the entire request in an autoreleasepool so ObjC/CF objects
        // from WindowInfo creation, AX API calls, and CGS bridging are
        // released promptly on background resolver threads.
        autoreleasepool {
            pidBody(for: window)
        }
    }

    /// The Accessibility identifier resolved for the given window's status
    /// item during the last PID scan (e.g. "com.apple.menuextra.wifi"), or nil
    /// if none was captured. Used to name Control Center modules that have no
    /// CG window title. Pure cache read — does not trigger a scan.
    func axIdentifier(for windowID: CGWindowID) -> String? {
        state.withLock { $0.axIdentifiers[windowID] }
    }
    ...
    private func pidBody(for window: WindowInfo) -> pid_t? {
        ...
        guard isTrusted else {
            SourcePIDCache.diagLog.warning("SourcePIDCache.pid: AXHelpers.isProcessTrusted() returned false — accessibility permission missing in current process")
            return nil
        }

        SourcePIDCache.diagLog.debug("SourcePIDCache.pid: performing batch resolution via AX API")
        if state.withLock({ $0.apps.isEmpty }) {
            SourcePIDCache.diagLog.debug("SourcePIDCache.pid: app cache empty, performing initial cleanup before AX scan")
            performCleanup()
        }
        ...
        for app in apps {
            ...
            autoreleasepool {
                ...
                for child in children {
                    ...
                    // false), hiding it from saved-layout management. Leaving it
                    // unresolved lets the marker-pair pass below supply the
                    // real owner PID; named CC items (BentoBox-0, Clock, WiFi,
                    // NowPlaying) carry non-generic titles and resolve to
                    // Control Center normally.
                    if let matchedWindow = allWindows.first(where: {
                        $0.bounds.center.distance(to: childCenter) <= SpatialMatchPolicy.strictRadius
                        ...
                        let pid = app.processIdentifier
                        // Capture the AX identifier for Control-Center-hosted
                        // items (Wi-Fi/Bluetooth/Clock/…) whose CG window title
                        // is empty. It's the only stable name source without
                        // Screen Recording. Only queried for CC-owned items to
                        // avoid an extra AX round-trip on every third-party item.
                        let axIdentifier = matchedWindow.title?.isEmpty != false && app.bundleIdentifier == ccBundleID
                            ? AXHelpers.identifier(for: child)
                            : nil
                        state.withLock {
                            $0.pids[matchedWindow.windowID] = pid
                            if let axIdentifier {
                                $0.axIdentifiers[matchedWindow.windowID] = axIdentifier
                            }
                        }
                    }
                }
        ...
        if !unresolvedWindows.isEmpty {
            var candidatesByWindow =
                [CGWindowID: [(distance: CGFloat, app: CachedApplication)]]()
            for app in apps {
                autoreleasepool {
                    guard let bar = app.getOrCreateExtrasMenuBar() else {
                        return
                    ...
                    for child in children {
                        ...
                        for window in allWindows where unresolvedWindows.contains(window.windowID) {
                            let distance = window.bounds.center.distance(to: childCenter)
                            guard distance <= SpatialMatchPolicy.clearNearestCandidateRadius else {
                                continue
                            }
                            candidatesByWindow[window.windowID, default: []].append((distance, app))
                        }
                    }
                }
            }
            ...
            for window in allWindows where unresolvedWindows.contains(window.windowID) {
                let candidates = (candidatesByWindow[window.windowID] ?? [])
                    .sorted { $0.distance < $1.distance }
                guard let best = candidates.first else {
                    continue
                }

                let secondDistance = candidates.dropFirst().first?.distance
                guard SpatialMatchPolicy.acceptsClearNearest(
                    bestDistance: best.distance,
                    secondDistance: secondDistance,
                    bestBundleID: best.app.bundleIdentifier,
                    windowTitle: window.title,
                    controlCenterBundleID: ccBundleID
                ) else {
                    continue
                }
                ...
                let pid = best.app.processIdentifier
                let appLabel = best.app.bundleIdentifier ?? best.app.localizedName ?? "unknown"
                state.withLock { $0.pids[window.windowID] = pid }
                SourcePIDCache.diagLog.info(
                    "SourcePIDCache clear-nearest spatial resolution: windowID=\(window.windowID) → " +
                        "PID \(pid) via \(appLabel) at distance \(best.distance)"
                )
            }
        }
        ...
        // CG-layer owning PID (when it is neither Continuum itself nor
        // Control Center) or by looking up the running app named by
        // the marker's bundle-ID title. Multi-match cases are skipped
        // to prevent misattribution. Continuum's own control items and
        // self-registration windows are excluded so Continuum's PID can
        // never be attributed to a third-party widget.
        if !unresolvedWindows.isEmpty {
            let continuumBundleID = "com.thepatientzero.Continuum"
            let markers = MarkerPairResolver.extractMarkers(
                ...
                continuumControlItemPrefix: "Continuum.ControlItem.",
