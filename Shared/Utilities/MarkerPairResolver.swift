...
enum MarkerPairResolver {
    ...
    struct Marker: Equatable {
        ...
        /// resolves to a bundle ID that is not Control Center or Continuum.
        let owningPID: pid_t?
    }
    ...
    /// are skipped to prevent misattribution. Continuum and Control Center
    ...
    ///     items-only list. Callers are expected to pre-filter Continuum
    ///     control items and the Continuum self-registration window.
    ///   - continuumBundleID: Continuum's own bundle identifier; excluded from
    ...
    /// (bundle-identifier shape), is not a Continuum control item, and is
    /// not the Continuum self-registration window.
    static func extractMarkers(
        from windows: [(windowID: CGWindowID, title: String?, size: CGSize, owningPID: pid_t?)],
        continuumControlItemPrefix: String,
        continuumBundleID: String
    ) -> [Marker] {
        windows.compactMap { window in
            guard let title = window.title, title.contains(".") else { return nil }
            if title.hasPrefix(continuumControlItemPrefix) { return nil }
            if title == continuumBundleID { return nil }
            return Marker(
    ...
    /// it from saved-layout management. The window must be left unresolved so
    /// marker-pair can supply the real owner PID. Named CC items (BentoBox-0,
    /// Clock, WiFi, NowPlaying, ...)
