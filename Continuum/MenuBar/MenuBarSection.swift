...
final class MenuBarSection {
    ...
    /// A Boolean value that indicates whether the Overlay Tray should be used
    /// on the current active display.
    private var useOverlayTray: Bool {
        guard let appState else { return false }
        let screen = screenForOverlayTray
        let displayID = screen?.displayID ?? CGMainDisplayID()
        return appState.settings.displaySettings.useOverlayTray(for: displayID)
    }
    ...
    private func presentationMode(on screen: NSScreen) -> PresentationMode {
        guard let appState else { return .overlayTray }
    ...
    /// The best screen to show the Overlay Tray on.
    ///
    /// Always returns the screen with the active menu bar so that
    /// clicking icons in the OverlayTray actually activates their popups.
    private weak var screenForOverlayTray: NSScreen? {
    ...
    var isHidden: Bool {
        if useOverlayTray {
            ...
            switch name {
            case .visible, .hidden:
                return menuBarManager?.overlayTrayPanel.currentSection != .hidden
            case .alwaysHidden:
                return menuBarManager?.overlayTrayPanel.currentSection != .alwaysHidden
            }
        }
        switch name {
        case .visible, .hidden:
            if menuBarManager?.overlayTrayPanel.currentSection == .hidden {
                return false
            }
            return desiredState == .hideSection
        case .alwaysHidden:
            if menuBarManager?.overlayTrayPanel.currentSection == .alwaysHidden {
                return false
            }
            return desiredState == .hideSection
        }
    ...
    func updateControlItemState(for screen: NSScreen? = nil) {
        ...
        let useOverlayTray = displaySettings.useOverlayTray(for: activeScreen.displayID)
        ...
        if name == .hidden || name == .visible, alwaysShow, !useOverlayTray {
            controlItem.state = .showSection
        } else {
            controlItem.state = desiredState
        }
    ...
    func show(triggeredByHotkey: Bool = false) {
        ...
        // Determine whether we should use the Overlay Tray based on settings.
        let shouldUseOverlayTrayBasedOnSettings = useOverlayTray

        let preferredPresentationMode: PresentationMode
        if shouldUseOverlayTrayBasedOnSettings {
            preferredPresentationMode = .overlayTray
        } else if let screen = screenForOverlayTray {
            ...
            case .overlayTray:
                diagLog.info("Not enough space to show items inline, falling back to Overlay Tray")
            }
        } else {
            preferredPresentationMode = .inline
        }

        // Use the overlay tray if settings say so OR if items still won't fit inline.
        if preferredPresentationMode == .overlayTray {
            // Make sure hidden and always-hidden control items are collapsed.
            // Still update the visible control item (control icon) state to show
            // its alternate icon.
            ...
            if let screen = screenForOverlayTray {
                switch name {
                case .visible, .hidden:
                    menuBarManager.overlayTrayPanel.show(
                        section: .hidden,
                        on: screen,
        ...
        // If we made it here, we're not using the Overlay Tray.
        // Make sure it's closed.
        menuBarManager.overlayTrayPanel.close()
    ...
    func hide() {
        ...
        menuBarManager.overlayTrayPanel.close() // Make sure Overlay Tray is always closed.
    ...
    /// OverlayTray panel, meaning the section should not be rehidden yet.
    private func isMouseInsideActiveArea() -> Bool {
        ...
        if appState.hidEventManager.isMouseInsideOverlayTray(appState: appState) {
            return true
        }
        return false
    }
    ...
    private func startRehideChecks() {
        ...
        case .smart:
            ...
            rehideTask = Task { [weak self, weak appState] in
                ...
                // Don't rehide while the mouse is inside the menu bar or OverlayTray.
