...
struct MenuBarItemTag: Hashable, CustomStringConvertible {
    ...
    var isSystemItem: Bool {
        switch namespace {
        case .controlCenter, .systemUIServer, .textInputMenuAgent, .weather, .passwords, .screenCaptureUI, .ssMenuAgent, .continuum, .gamePolicyAgent:
            return true
    ...
    var isMovable: Bool {
        !MenuBarItemTag.immovableItems.contains(where: { immovable in
            guard immovable.namespace == namespace else { return false }
            ...
            // item misses the "Item-0" tag, is treated as movable, and every
            // Save times out trying to drag an item macOS refuses to move.
            if immovable.namespace == .controlCenter {
                return immovable.title == title
            }
            return true
        })
    }
    ...
    /// A Boolean value that indicates whether the item identified
    /// by this tag is a control item owned by Continuum.
    ...
    private init(controlItem identifier: ControlItem.Identifier) {
        self.init(namespace: .continuum, title: identifier.rawValue, instanceIndex: 0)
    }
}
...
extension MenuBarItemTag {
    ...
    /// An array of tags for items representing Continuum's control items.
    static let controlItems = ControlItem.Identifier.allCases.map(\.tag)

    // MARK: Control Items

    /// The tag for Continuum's control item for the "Visible" section.
    static let visibleControlItem = MenuBarItemTag(controlItem: .visible)

    /// The tag for Continuum's control item for the "Hidden" section.
    static let hiddenControlItem = MenuBarItemTag(controlItem: .hidden)

    /// The tag for Continuum's control item for the "Always-Hidden" section.
...
extension MenuBarItemTag.Namespace {
    /// The namespace for the "Continuum" process.
    static let continuum = string(Constants.bundleIdentifier)
