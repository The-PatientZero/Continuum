...
/// any specific trigger (saved-section restore, reset reconciliation, etc.).
///
/// DesiredLayout is the value type that gives savedSectionOrder a typed
/// per-section shape plus a MenuBarNewItemsPlacement fallback for items the
/// user has never seen before. The reconciler compares this against an
/// ObservedLayout and emits the moves needed to make reality match desire.
struct DesiredLayout: Equatable {
    ...
    var newItemsPlacement: MenuBarNewItemsPlacement

    /// Builds a DesiredLayout from a persisted savedSectionOrder
    /// dictionary (string-keyed) and a MenuBarNewItemsPlacement preference.
...
enum LayoutReconciler {
    ...
    /// to produce a concrete `MenuBarMoveDestination`.
    ///
    ...
    /// positions and falling back to the MenuBarNewItemsPlacement preference.
    ///
    ...
    /// uniqueIdentifier and consumed by the saved-layout orchestrator to
