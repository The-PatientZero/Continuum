...
enum LayoutSolver {
    ...
    enum LeftmostMove: Equatable {
        /// The Continuum visible-control icon is sitting left of the hidden
        /// divider; restore it to the visible section.
        case controlIcon(MenuBarItem)
        /// A non-hideable system item (capture / mic / camera
    ...
    /// A placement decision for an unmanaged item during saved-layout apply.
    ///
    /// Encodes intent (saved vs new-item-default vs new-item-anchored)
    /// without committing to a concrete `MenuBarMoveDestination`. The
    ...
    enum UnmanagedPlacement: Equatable {
        ...
        case newItemAnchored(
            ...
            anchorUID: String,
            relation: MenuBarNewItemsPlacement.Relation
        )
    }
    ...
    /// sequence the saved-layout planner consumes, inserting the hidden and
    ...
    ///
    /// Pure over its inputs. Shared by applySavedSectionLayout and the log-replay
    ...
    /// are neither in the desired saved-layout sequence nor any of the
    /// three Continuum control items.
    ///
    ...
    /// through MenuBarNewItemsPlacement and the LCS planner will emit moves
    /// that drag the Continuum icon to the user's configured anchor on every
    /// cache cycle. Visible-control-item exclusion was the omission
    /// that caused the field-reported "Continuum icon keeps moving" bug.
    ///
    ...
    /// hosts under Control Center that Continuum cannot yet attribute to their
    ...
    /// never match a saved-layout entry, and would otherwise be relocated as
    ...
    /// (1) Continuum visible-control icon recovery, (2) non-hideable system
    ...
    static nonisolated func planLeftmostMove(
        ...
        // Items sitting left of the hidden divider. The Continuum icon is a
        // control item but must always be visible, so we admit it here.
        // In the hidden state macOS can report the expanded divider as a huge
        // off-screen window; the Continuum icon may overlap its minX by a
        // point while still being classified as hidden. Use the already
        // computed section classification for that recovery path.
        func isLeftOfHiddenBoundary(_ item: MenuBarItem) -> Bool {
            if item.bounds.maxX <= observation.hiddenBounds.minX {
                return true
            }
            guard item.tag == .visibleControlItem,
                  observation.sectionByWindowID[item.windowID] != .visible
            else {
                return false
            }
            return item.bounds.midX < observation.hiddenBounds.midX
        }

        let leftmostItems = items
            .filter {
                isLeftOfHiddenBoundary($0) &&
                    $0.isMovable &&
        ...
        // Path 1: Continuum icon.
        if let controlIcon = leftmostItems.first(where: { $0.tag == .visibleControlItem }) {
            return .controlIcon(controlIcon)
        }

        // Path 2: non-hideable system item (camera / mic / capture).
        ...
        if let systemItem = leftmostItems.first(where: {
            !$0.canBeHidden &&
                ...
                !$0.isUnresolvedControlCenterPlaceholder
        }) {
        ...
        // Unresolved identity short-circuit. Without stable identity,
        // third-party items hosted by Control Center can fall back to
        // namespace com.apple.controlcenter, which prevents matching
        // against savedSectionOrder (real bundle IDs). The next cache
        // pass with resolved sourcePIDs will handle relocation safely.
        if hideableLeftmost.contains(where: { !$0.identityConfidence.allowsAutomatedMove }) {
    ...
    /// (newly-detected, not in the saved layout) are the first candidates
    /// to overflow because the saved layout has no stored position for
    /// them. Saved-layout items only overflow if even removing all
    ...
    static nonisolated func planNotchOverflow(
        ...
        // Visible-section UIDs in saved order (left-to-right).
        ...
        let savedNonChevron = nonChevronUIDs.filter { !unmanagedSet.contains($0) }

        // Saved-layout baseline: chevron + all saved visible items.
        var savedBaseline: CGFloat = chevronWidth
        for uid in savedNonChevron {
            savedBaseline += uidWidths[uid] ?? 0
        }

        var overflowUIDs: [String] = []

        if savedBaseline > availableWidth {
            // Saved layout alone exceeds budget. All unmanaged overflow
            // plus enough saved items (leftmost first) to fit. Iterate
            // saved items from the CC end inward; whatever doesn't fit
            // overflows.
            overflowUIDs.append(contentsOf: unmanagedNonChevron)
            var savedFitting = [String]()
            var usedWidth = chevronWidth
            for uid in savedNonChevron.reversed() {
                let width = uidWidths[uid] ?? 0
                if usedWidth + width <= availableWidth {
                    usedWidth += width
                    savedFitting.insert(uid, at: 0)
                } else {
                    break
                }
            }
            let savedOverflow = Array(
                savedNonChevron.prefix(savedNonChevron.count - savedFitting.count)
            )
            overflowUIDs.append(contentsOf: savedOverflow)
        } else {
            // Saved layout fits. Try to fit unmanaged items from the CC
            // end; whatever doesn't fit overflows. Saved items stay put.
            var usedWidth = savedBaseline
    ...
    // MARK: - Visible boundary correction

    /// Plans moves for items whose desired section differs from their
    /// observed section across the visible/tray boundary.
    ///
    /// The LCS reorder pass intentionally removes divider controls before it
    /// compares current and desired order. That makes pure order comparison
    /// blind to this common case:
    ///
    ///     current: [A, hidden divider, B]
    ///     desired: [A, B, hidden divider]
    ///
    /// Once controls are removed, both sides look like [A, B], so LCS emits
    /// no move even though B is on the wrong side of the divider. This planner
    /// catches those section-only differences and lets the orchestrator move
    /// the item to the target section boundary before the normal LCS ordering
    /// pass runs.
    static nonisolated func planVisibleBoundaryMoves(
        currentSectionByUID: [String: MenuBarSection.Name],
        desiredSectionByUID: [String: MenuBarSection.Name],
        desiredOrder: [String]
    ) -> [LCSPlannedMove] {
        var seen = Set<String>()
        let orderedUIDs = desiredOrder + currentSectionByUID.keys.sorted()
        var result = [LCSPlannedMove]()

        for uid in orderedUIDs where !seen.contains(uid) {
            seen.insert(uid)
            guard
                let currentSection = currentSectionByUID[uid],
                let desiredSection = desiredSectionByUID[uid],
                currentSection != desiredSection,
                currentSection == .visible || desiredSection == .visible
            else {
                continue
            }
            ...
            result.append(
                LCSPlannedMove(
                    uid: uid,
                    destination: .sectionBoundary(desiredSection)
                )
            )
        }
    ...
    /// Decides where each unmanaged item should land during a saved-layout
    /// apply, consulting saved positions first and falling back to the
    /// user's MenuBarNewItemsPlacement preference.
    ///
    /// Unmanaged items are items present in the live menu bar but not
    /// covered by the saved layout. Today's behavior parks them all at
    ...
    static nonisolated func planUnmanagedPlacement(
        ...
        for uid in unmanagedUIDs {
            ...
            // 2. MenuBarNewItemsPlacement anchor (if configured and present in
    ...
    ///
    /// Used by the saved-layout unmanaged placement path
    /// (applySavedSectionLayout's unmanaged-items block via
