...
final class ControlItem {
    ...
    enum Identifier: String, CaseIterable {
        ...
        case visible = "Continuum.ControlItem.Visible"
        /// The identifier for the control item for the hidden section.
        case hidden = "Continuum.ControlItem.Hidden"
        /// The identifier for the control item for the always-hidden section.
        case alwaysHidden = "Continuum.ControlItem.AlwaysHidden"
    ...
    /// Returns the status item's current window, resolving it directly from
    /// NSStatusItem when the published KVO-backed value has not populated yet.
    func resolvedWindow() -> NSWindow? {
        if let window {
            return window
        }

        guard let resolvedWindow = statusItem.button?.window else {
            return nil
        }
    ...
    private func configureCancellables() {
        ...
        if let appState {
            ...
            if identifier == .visible {
                appState.settings.general.$showControlIcon
                    .removeDuplicates()
                    .receive(on: DispatchQueue.main)
                    .sink { [weak self] shouldShow in
                        ...
                        setControlIconDisplayed(shouldShow)
                    }
                    .store(in: &c)

                appState.settings.general.$controlIcon
                    .combineLatest(appState.settings.general.$customControlIconIsTemplate)
                    .removeDuplicates()
                    .receive(on: DispatchQueue.main)
                    .sink { [weak self] _ in
                        self?.updateStatusItem()
    ...
    private func updateStatusItem() {
        ...
        switch identifier {
        case .visible:
            if !appState.settings.general.showControlIcon {
                hideControlIconCompletely()
                return
            }
            updateStatusItemVisibility(true)
            button.appearsDisabled = false

            let icon = appState.settings.general.controlIcon
    ...
    private func setControlIconDisplayed(_ shouldShow: Bool) {
        statusItem.isVisible = true
        if shouldShow {
            updateStatusItem()
            return
    ...
    /// Hides the control icon without removing the status item or losing autosave data.
    private func hideControlIconCompletely() {
        constraint?.isActive = false
        statusItem.length = 0
    ...
    private func createMenu(with appState: AppState) -> NSMenu {
        ...
        if
            ...
            section.isEnabled
        {
            let sectionTitle = section.isHidden
            ...
            let iconName = section.isHidden ? "eye" : "eye.slash"
