...
enum MouseHelpers {
    ...
    private static func forceShowCursor(reason: String) {
        ...
        // Re-couple before showing so the watchdog can never leave the mouse
        // disassociated from the cursor (see hideCursor for why we decouple).
    ...
    // TEMP DEBUG: short call-stack hint to identify who toggled the cursor.
    private static func callerHint() -> String {
        Thread.callStackSymbols.dropFirst(2).prefix(3)
            .map { line in
                let parts = line.split(separator: " ", omittingEmptySubsequences: true)
                return parts.count > 3 ? String(parts[3...].joined(separator: " ").prefix(70)) : line
            }
            .joined(separator: " <- ")
    }
    ...
    static func hideCursor(watchdogTimeout: DispatchTimeInterval? = nil) {
        ...
        cursorLock.sync {
            cursorHideCount += 1
            count = cursorHideCount
            shouldHide = cursorHideCount == 1
        }
        diagLog.info("CURSOR hide: count=\(count) willHide=\(shouldHide) caller=\(callerHint())")
        ...
        if result != .success {
            ...
            // While the cursor is hidden for synthesized input (menu bar item
            // drags/clicks), decouple the physical mouse from the cursor.
            // CGWarpMouseCursorPosition otherwise drags the *visible* pointer to
            // the warp target and the still-associated physical mouse instantly
            // tugs it back, producing one visible flash per moved item. With the
            // pair disassociated the warp only moves the logical hit-test point,
            // so nothing is rendered. Re-coupled in showCursor / forceShowCursor.
            associateMouseAndCursor(false)
            diagLog.info("CURSOR decouple (mouse<->cursor disassociated)")
    ...
    static func showCursor() {
        ...
        diagLog.info("CURSOR show: count=\(count) willShow=\(shouldShow) wasZero=\(wasAlreadyZero) caller=\(callerHint())")
        ...
        // Re-couple the physical mouse and cursor before showing (Apple's
        // documented order: associate(true) then ShowCursor). Balances the
        // associateMouseAndCursor(false) performed on the matching hide.
        associateMouseAndCursor(true)
        diagLog.info("CURSOR recouple (mouse<->cursor re-associated)")
    ...
    static func warpCursor(to point: CGPoint) {
        diagLog.info("CURSOR warp: to=(\(Int(point.x)),\(Int(point.y))) caller=\(callerHint())")
        let result = CGWarpMouseCursorPosition(point)
        if result != .success {
            diagLog.warning("CURSOR warp FELL BACK to mouseMoved event to=(\(Int(point.x)),\(Int(point.y))) (warp error: \(result.rawValue))")
