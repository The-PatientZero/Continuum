...
final class HIDEventManager: ObservableObject {
    ...
    private func rebuildWindowBoundsLookup(from cache: MenuBarItemCache) {
    ...
    private func configureCancellables() {
        ...
        if let appState {
            // Pre-seed so the initial CombineLatest emission is treated as a
            ...
            // Start or stop the mouse-moved tap when show-on-hover or
            // per-display configurations change.
            Publishers.CombineLatest(
                appState.settings.general.$showOnHover,
                appState.settings.displaySettings.$configurations
            )
            .sink { [weak self] showOnHover, _ in
                guard let self, isEnabled else {
                    return
                }
                ...
                // rearm path when display config changes while showOnHover was
                // already enabled.
...
extension HIDEventManager {
    ...
    private func handleShowOnClick(appState: AppState, screen: NSScreen, clickLocation: CGPoint, modifierFlags: NSEvent.ModifierFlags, isDoubleClick: Bool = false) {
        ...
        // on a widget's icon don't also trigger Continuum's show-on-click reveal.
        ...
        if isDoubleClick {
            ...
            {
                // If the always-hidden section is currently showing via the Continuum
                // Bar, a plain click should close it rather than switch the Continuum
                // Bar to the hidden section.
                if appState.menuBarManager.overlayTrayPanel.currentSection == .alwaysHidden,
                   let alwaysHiddenSection = appState.menuBarManager.section(withName: .alwaysHidden)
                {
                    disarmShowOnClickGuard()
    ...
    private func handleSmartRehide(
        ...
        // Make sure clicking the control icon doesn't trigger rehide.
        if let controlIcon = appState.menuBarManager.controlItem(withName: .visible) {
            guard event.window !== controlIcon.window else {
                return
            }
        }

        // Only continue if the click is not inside the Overlay Tray, at
        // least one section is visible, and the mouse is not inside
        // the menu bar.
        guard
            event.window !== appState.menuBarManager.overlayTrayPanel,
    ...
    /// using the system-wide accessibility hit-test. Excludes Continuum's own
    ...
    /// role). When the helper returns true, Continuum defers to the widget under
    ...
    private func isCursorOverForeignWidgetUIElement() -> Bool {
        ...
        if AXHelpers.rawRole(for: element) == "AXMenuBarItem" {
            return false
        }
    ...
    /// Returns whether any non-Continuum window at the pop-up-menu window level is
    ...
    /// overlay, which is needed to avoid showing Continuum's secondary context
    ...
    ///
    /// After a saved-layout restore with OverlayTray active, the Window Server
    /// tracks the expanded control item windows and routes clicks to them
    /// instead of the application menus underneath. This method uses AX to
    /// locate the correct menu bar item, then posts a synthetic click directly
    /// to the owning application's PID.
    ///
    ...
    private func handleShowOnHover(appState: AppState, screen: NSScreen) {
        ...
        if hiddenSection.isHidden {
            ...
            guard
                !isMouseInsideMenuBar(appState: appState, screen: screen),
                !isMouseInsideOverlayTray(appState: appState)
            else {
    ...
    private func handlePreventShowOnHover(
        ...
        guard
            appState.settings.general.showOnHover,
            !appState.settings.displaySettings.useOverlayTray(for: screen.displayID)
        else {
            return
        }
        ...
        if isMouseInsideMenuBarItem(appState: appState, screen: screen) {
            ...
            case .leftMouseDown:
                ...
                if isMouseInsideControlIcon(appState: appState) {
                    break
                }
                return
            case .rightMouseDown:
...
extension HIDEventManager {
    /// Returns the best screen to use for hover and scroll calculations.
    ///
    /// Always returns the screen that currently owns the active menu bar.
    /// This prevents showing the hidden section or OverlayTray on a monitor
    ...
    private func isMouseInsideCachedMenuBarItem() -> Bool {
        ...
        for entry in entries {
            ...
            // Verify window still exists — temporary items (capture,
        ...
        -> Bool
    {
        ...
        return !isInAppMenu
            && !isMouseInsideMenuBarItem(appState: appState, screen: screen)
            && !isMouseInsideControlIcon(appState: appState)
    }

    /// A Boolean value that indicates whether the mouse pointer is within
    /// the bounds of the Overlay Tray panel.
    func isMouseInsideOverlayTray(appState: AppState) -> Bool {
        guard let mouseLocation = MouseHelpers.locationAppKit else {
            return false
        }
        let panel = appState.menuBarManager.overlayTrayPanel
        // Pad the frame to be more forgiving if the user accidentally
        // moves their mouse outside of the Overlay Tray.
        let paddedFrame = panel.frame.insetBy(dx: -15, dy: -15)
        return paddedFrame.contains(mouseLocation)
    }

    /// A Boolean value that indicates whether the mouse pointer is within
    /// the bounds of the control icon.
    func isMouseInsideControlIcon(appState: AppState) -> Bool {
        guard
            ...
            let controlIconFrame = visibleSection.controlItem.window?.frame,
            let mouseLocation = MouseHelpers.locationAppKit
        else {
            return false
        }
        return controlIconFrame.contains(mouseLocation)
    }
